...
CREATE TABLE fact_sales
  (
     ...
     line_number    nvarchar(25) NOT NULL,
     quantity       smallint,
     unit_price     MONEY,
     unit_cost      MONEY,
     tax_amount     MONEY,
...
IF EXISTS (SELECT *
  ...
  DROP INDEX fact_sales_dim_product ON fact_sales;
...
IF EXISTS (SELECT *
           ...
           WHERE  NAME = 'fact_sales_dim_customer'
                  AND object_id = Object_id('fact_sales'))
  DROP INDEX fact_sales_dim_customer ON fact_sales;
...
IF EXISTS (SELECT *
           ...
           WHERE  NAME = 'fact_sales_dim_territory'
                  AND object_id = Object_id('fact_sales'))
  DROP INDEX fact_sales_dim_territory ON fact_sales;
...
IF EXISTS (SELECT *
           ...
           WHERE  NAME = 'fact_sales_dim_date'
                  AND object_id = Object_id('fact_sales'))
  DROP INDEX fact_sales_dim_date ON fact_sales;
